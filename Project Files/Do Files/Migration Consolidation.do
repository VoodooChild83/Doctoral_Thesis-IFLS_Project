// Append all Migration Datasets to consolidate the events
...
foreach year in 1993 1997 2000 2007{
		quietly do "$maindir$project/Do Files/Migration `year'.do"
		}
...
* For those with all missing values in the year 12 location, assume they live in the same location as in birth locaation
...
sort pidlink MigYear wave movenum panel93 panel97 panel00 panel07
...
foreach rep in surv /*kec kab*/ {
		...
		preserve
			...
			* Append in the Birth-Age12 dataset to set initial point based from age 12 location
				...
				sort pidlink MigYear stage movenum panel93 panel97 panel00 panel07
			...
			* for recorded pre-age 12 migrations
			...
			save "$maindir$tmp/MigrationEvents-Rep`rep'Drop.dta", replace
...
foreach rep in surv /*kec kab*/{
	...
	use "$maindir$tmp/MigrationEvents-Rep`rep'Drop.dta"
	...
	* Initialize the Counting Cycles
	...
	quietly levelsof TotalMoves, local(levels)
	...
	* First, find the inter-geographical migration events
	...
	quietly foreach var in kec kab prov UrbRur{
			...
			foreach l of local levels{
					by pidlink: replace flag_`var'mig=1 if TotalMoves==`l' & `var'mov[_n-1]!=`var'mov[_n] /* & `var'[_n-1]!=. & `var'mov[_n]!=.*/
					}
			}
	...
	* Second, find the intra-geographical migration events (that is, events that occur intraregency (implies same province) and/or intraprovincially)
	...
	quietly foreach var in kec kab vill{
				...
				if "`var'"=="kec"|"`var'"=="kab"{
					foreach l of local levels{
							by pidlink (stage): replace flag_`var'migsame=1 if TotalMoves==`l' & `var'mov[_n-1]!=`var'mov[_n] & id[_n-1]==id[_n] /*& `var'mov[_n-1]!=. & `var'mov[_n]!=.*/
							}
				drop id
				}
				if "`var'"=="vill"{
					foreach l of local levels{
							by pidlink (stage): replace flag_`var'migsame=1 if TotalMoves==`l' & id[_n-1]==id[_n] 
							}
				drop id
				}
		...
		*drop movenumMaster
		...
		rename (*provmig *kecmigsame *kabmigsame *villmigsame) (*InterProvMig *IntraKabMig *IntraProvMig *IntraKecMig)

	save "$maindir$tmp/MigrationEvents-Rep`rep'Drop.dta", replace
	save "$maindir$tmp/MigrationEvents-Rep`rep'Drop-For Mig Year Dummeis.dta", replace


	* Count the types of migration events that occur

		quietly foreach var in IntraKecMig InterProvMig IntraKabMig IntraProvMig UrbRur{
					foreach year in 1993 1997 2000 2007{
	...
	* Repeat for Cohorts
		...
		* Drop the inconsistent Migration Event
		...
		drop TotalEvents flag_InconsisMov* flag Cohort
	...
	save "$maindir$tmp/MigrationEvents-Rep`rep'DropConsolidated.dta", replace
