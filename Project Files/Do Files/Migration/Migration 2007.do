// Migration Information 2007

********************************************************************************
//Current Residence Locations
use "$maindir$wave_4/bk_sc.dta", clear

rename (sc030707 sc020707 sc010707) (kec_curr kab_curr prov_curr)

keep hhid07 kec* kab* prov*

save "$maindir$tmp/res2007.dta", replace

********************************************************************************
// 2007 Birth And Age 12 Clean for both main and proxy modules:

foreach let in a p{

	if "`let'"=="a" use "$maindir$wave_4/b3`let'_mg1.dta"
	else use "$maindir$wave_4/b`let'_mg.dta"

	* First, merge in current residence file to the birthplace and year 12 dataset
	
		merge m:1 hhid07 using "$maindir$tmp/res2007.dta"
		drop if _merge==2
		drop _merge
	...
	* Prov
	...
	if "`let'"=="a"{
		...
		rename mg18a panel07
		label define panel 1 "Panel, preprinted" 2 "Panel, not preprinted" 3 "New"
		label values panel07 panel

		* Reshape Long
			keep hhid07 pid07 pidlink kec* kab* prov* panel07 UrbRur* book
			drop *_curr
		...
		* Reshape Long
			keep hhid07 pid07 pidlink kec* kab* prov* UrbRur* book
			drop *_curr

			reshape long kecmov@ kabmov@ provmov@ UrbRurmov@, i(pidlink) j(stage)
		}	

	if "`let'"=="a" save "$maindir$tmp/b3`let'_mg1_2007.dta", replace
	else save "$maindir$tmp/b3`let'_mg1_2007.dta", replace
}

erase "$maindir$tmp/res2007.dta"

********************************************************************************
// Append the Proxy and Main datasets: Birth-Age12 data

use "$maindir$tmp/b3a_mg1_2007.dta"

append using "$maindir$tmp/b3p_mg1_2007.dta"

erase "$maindir$tmp/b3p_mg1_2007.dta"
...
save "$maindir$tmp/b3a_mg1_2007.dta", replace

********************************************************************************
// Clean the Main and Proxy Migration Event Datasets

// Main 2007 Migration DataSet

use "$maindir$wave_4/b3a_mg2.dta"

keep hhid07 pid07 pidlink movenum mg21bx-mg23b mg24yr mg25 mg26 mg35 mg36
gen book="a"
...
save "$maindir$tmp/b3a_mg2_2007.dta", replace

// Proxy 1007 Migration Dataset

use "$maindir$wave_4/bp_mg.dta"

keep pid07 hhid07 pidlink mg05eyr mg05f mg05gd-mg05gb
...
gen book="p"
	...
	* Kec
	gen kecmov=mg05gb if !(mg05gb>900)
	
	* Kab
	gen kabmov=mg05gc if !(mg05gc>90)
	
	* Prov
	gen provmov=mg05gd if !(mg05gd>94)
	
drop mg05gd-mg05gb

drop if provmov==. & kecmov==. & kabmov==.

save "$maindir$tmp/b3p_mg2_2007.dta", replace

********************************************************************************
// Append the Proxy and Main datasets: After 12 years old migration

use "$maindir$tmp/b3a_mg2_2007.dta"

append using "$maindir$tmp/b3p_mg2_2007.dta"

erase "$maindir$tmp/b3p_mg2_2007.dta"
...
// Merge birthday information to get full years for all people
	  ...
	  foreach mig in 1 2{
			use "$maindir$tmp/b3a_mg`mig'_2007.dta"

	   		merge m:1 pidlink using "$maindir$tmp/birthyear.dta"
	   		drop if _merge==2
	   		drop _merge
	   		
	   		save "$maindir$tmp/b3a_mg`mig'_2007.dta", replace
	   		}
	   
	  erase "$maindir$tmp/birthyear.dta"

********************************************************************************
// Update 2007 Migration Event Years:
...
gen MigYear=mg24yr
	...
	* Update migration year using birth year and migration age
	...
	gen flag_MigAge=1 if (birthyr+MigAge>2008) & MigYear==. & MigAge!=. // migration event is beyond the latest wave year: 1998
...
preserve
	drop if stage==12
	keep pidlink panel07
	save "$maindir$tmp/panelinfo2007.dta", replace
restore

use "$maindir$tmp/b3a_mg2_2007.dta"

merge m:1 pidlink using "$maindir$tmp/panelinfo2007.dta"
drop if _merge==2
drop _merge flag_FinalObs

save "$maindir$tmp/b3a_mg2_2007.dta", replace

erase "$maindir$tmp/panelinfo2007.dta"
